...
const create = async (contatoData) => {
  ...
  const [result] = await pool.execute(query, [nome_completo, telefone, email || null, usuario_id || null])
...
const update = async (id, contatoData) => {
  ...
  if (contatoData.nome_completo !== undefined) {
    ...
    valores.push(contatoData.nome_completo ?? null)
  }

  if (contatoData.telefone !== undefined) {
    campos.push("telefone = ?")
    valores.push(contatoData.telefone ?? null)
  }

  if (contatoData.email !== undefined) {
    campos.push("email = ?")
    valores.push(contatoData.email ?? null)
  }
